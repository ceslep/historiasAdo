...
<div class="space-y-3">
  ...
  <!-- Indicador de búsqueda offline -->
  {#if query.length > 0 && !isSearchingOnline}
    <div class="mt-2 flex items-center gap-2 text-xs">
      {#if !isOnline}
        <span class="flex items-center gap-1 rounded-full bg-amber-100 border border-amber-200 px-2.5 py-0.5 text-amber-700 font-medium">
          <WifiOff class="h-3 w-3" />
          Búsqueda offline
        </span>
      {:else}
        <span class="flex items-center gap-1 rounded-full bg-blue-100 border border-blue-200 px-2.5 py-0.5 text-blue-700 font-medium">
          <Globe class="h-3 w-3" />
          Resultados locales
        </span>
      {/if}
  ...
  <!-- Indicador de sincronización inicial -->
  {#if isSyncing}
    <div class="mt-2 flex items-center gap-2 text-xs">
      <span class="flex items-center gap-1 rounded-full bg-indigo-100 border border-indigo-200 px-2.5 py-0.5 text-indigo-700 font-medium">
